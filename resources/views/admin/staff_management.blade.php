...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-4">
        ...
        <a href="{{ route('admin.staff.create') }}"
           class="bg-blue-500 text-white py-2 px-4 rounded-md">Add Staff</a>
    </div>
        ...
        <table class="min-w-full table-auto">
            ...
            @foreach($staffMembers as $staffMember)
                <tr class="border-b border-gray-200 hover:bg-gray-100">
                    <td class="py-3 px-6 text-left whitespace-nowrap">{{ $staffMember->name }}</td>
                    <td class="py-3 px-6 text-left">{{ $staffMember->email }}</td>
                    <td class="py-3 px-6 text-left">{{ $staffMember->user_type }}</td>
                    <td class="py-3 px-6 text-center">
                        <a href="{{ route('admin.staff.edit', $staffMember->id) }}" class="text-blue-500 hover:text-blue-700 mr-2">Edit</a>
                        <form action="{{ route('admin.staff.destroy', $staffMember->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
